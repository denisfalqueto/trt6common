...
import br.jus.trt.lib.common_tests.dataloader.UF_dd_DataLoader;
...
public class LoadDatasTest extends LocalTransactionTestBase {
	...
	public void loadDatasInMethodTest() {
		...
		count = getQuerier().executeCountQuery("select count(uf) from UF uf where uf.sigla=?", "cc");
		Assert.assertEquals(1, count);
		
		count = getQuerier().executeCountQuery("select count(uf) from UF uf where uf.sigla=?", "dd");
		Assert.assertEquals(1, count);
	}
	...
	public void precedenceTest() {
		 ...
		 */
		// buscando o registro para confirmação
		UF uf_aa = (UF) getQuerier().executeQuery("select uf from UF uf where uf.sigla=?", "aa").get(0); //p=1
		UF uf_cc = (UF) getQuerier().executeQuery("select uf from UF uf where uf.sigla=?", "cc").get(0); //p=2 
		UF uf_bb = (UF) getQuerier().executeQuery("select uf from UF uf where uf.sigla=?", "bb").get(0); //p=3
		UF uf_dd = (UF) getQuerier().executeQuery("select uf from UF uf where uf.sigla=?", "dd").get(0); //p=4 
