...
public class EntitySerializer implements Serializer {
    ...
    protected void serializeProperties(EntityType model,  SerializerConfig config,
            CodeWriter writer) throws IOException {
    	for (Iterator<Property> iterator = model.getProperties().iterator(); iterator
				.hasNext();) {
...
//                        writer.getClassConstant(localRawName), writer.getClassConstant(writer.getRawName(queryType)), inits);
//                
        		model = (EntityType) propertyType.getParameters().get(0);
        		propertyType = (EntityType) propertyType.getParameters().get(0);
        		queryType = typeMappings.getPathType(propertyType, model, false);
        		
        		Property property2 = new Property(property.getDeclaringType(), property.getName(), propertyType);
        		entityField(model, property2, config, writer);
        		
        		iterator.remove();
        		
        		entityAccessor(model, property2, writer);
